...
public class CoreManager {
    ...
    public static LinkedList<Integer> loadJava(Class<?> annotItfClass) {
        ...
        for (java.lang.reflect.Method m : annotItfClass.getDeclaredMethods()) {
            ...
            //System.out.println("Reading method " + m.getName());
            ...
            if (m.isAnnotationPresent(integratedtoolkit.types.annotations.Method.class)) {
                ...
                if (m.isAnnotationPresent(integratedtoolkit.types.annotations.MultiConstraints.class)) {
                    ...
                    for (int i = 0; i < implementationCount; i++) {
                        ...
                        if (specificConstraints.processorCoreCount == 0) {
                            specificConstraints.processorCoreCount = 1;
                        }
                    ...
                    for (int i = 0; i < implementationCount; i++) {
                        ...
                        if (defaultConstraints.processorCoreCount == 0) {
                            defaultConstraints.processorCoreCount = 1;
                        }
                    }
                }
    ...
    private static void loadMethodConstraints(int coreId, int implementationCount, String[] declaringClasses, Constraints[] cts) {
        ...
        for (int i = 0; i < implementationCount; i++) {
            ...
            if (cts[i] != null) {
                ...
                String hostQueue = cts[i].hostQueue();
                if (hostQueue.compareTo("[unassigned]") != 0) {
                    int last = 0;
                    while (hostQueue.length() > 0) {
                        last = hostQueue.lastIndexOf(",");
                        rm.addHostQueue(hostQueue.substring(last + 1, hostQueue.length()));
                        if (last == -1) {
                            hostQueue = "";
                        } else {
                            hostQueue = hostQueue.substring(0, last);
                        }
                    }
                }
                ...
                if (software.compareTo("[unassigned]") != 0) {
                    ...
                    while (software.length() > 0) {
                        ...
                        String app = software.substring(last + 1, software.length());
        ...
        for (int method_id = 0; method_id < CoreManager.coreCount; method_id++) {
            ...
            for (int implementationId = 0; !executable && implementationId < annot[method_id].length; implementationId++) {
                ...
                if (mc != null) {
                    ...
                    //Check HostQueue
                    if (executable && mc.hostQueue().compareTo("[unassigned]") != 0) {
                        for (String elem : mc.hostQueue().split(",")) {
                            if (elem.compareTo("[unassigned]") != 0) {
                                if (!rd.getHostQueue().contains(elem)) {
                                    executable = false;
                                }
                            }
                        }
                    ...
                    //Check appSoftware
                    if (executable && mc.appSoftware().compareTo("[unassigned]") != 0) {
                        for (String elem : mc.appSoftware().split(",")) {
                            if (elem.compareTo("[unassigned]") != 0) {
                                if (!rd.getAppSoftware().contains(elem)) {
                                    executable = false;
                                }
                            }
                        }
