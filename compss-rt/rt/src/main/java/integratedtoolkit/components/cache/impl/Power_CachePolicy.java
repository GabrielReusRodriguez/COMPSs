 ...
 *
 */
public class Power_CachePolicy implements CachePolicy {
	...
	public CacheType selectCacheLevel(Parameter parametro,
		...
		// Solo decido si puedo saber que tamaño tiene y para eso necesito datos
		// del parametro
		long size = (long) -1.0d;

		PriorityQueue<PriorityQueueNode> cacheLevels = new PriorityQueue<Power_CachePolicy.PriorityQueueNode>(
				3);

		if (parametro instanceof DependencyParameter) {
			DependencyParameter dp = (DependencyParameter) parametro;
			
			//Solo si existe el fichero en el disco, buscamos su tamaño. Si no existe lo grabamos en disco para curarnos en salud
			if (cacheManager.existsPhysicalFile(dp, FTM)) {
				...
				if (cacheManager.isCacheLevelEnabled(CacheType.LEVEL1)) {
					calculateCacheScore(cacheManager, size, cacheLevels,CacheType.LEVEL1);
				}
				
				if (cacheManager.isCacheLevelEnabled(CacheType.LEVEL2)) {
					calculateCacheScore(cacheManager, size, cacheLevels,CacheType.LEVEL2);
				}

				if (cacheManager.isCacheLevelEnabled(CacheType.LEVEL3)) {
					calculateCacheScore(cacheManager, size, cacheLevels,CacheType.LEVEL3);				}
	...
	private void calculateCacheScore(CacheManager cacheManager, long size,
		...
		float ratio = calculateRatio(level, size,
				cacheManager);
		if (ratio < 0.9) {
	...
	private float calculateRatio(CacheType level, long fileSize,
		...
		float ratio = 0.0f;
		long freeSpace = cacheManager.getFreeSpace(CacheType.LEVEL1);
	...
	private long getSizeOfFile(DependencyParameter dp,
		...
		if (dp.getDirection() == ParamDirection.OUT) {
			...
			PhysicalDataInfo pdi = FTM.getPhysicalDataInfoFromDataInstance(waId
