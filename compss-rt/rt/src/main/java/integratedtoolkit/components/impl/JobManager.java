...
public class JobManager implements JobCreation, TransferStatus {
    ...
    public void cleanup() {
        ...
        if (tracing)
        	Tracer.fini(Core.signatureToId, System.getProperty(ITConstants.IT_SCRIPT_DIR), System.getProperty(ITConstants.IT_APP_NAME));
    ...
    private void submitJob(int jobId) {
        if (presched) {
            Job job = idToJob.get(jobId);
            String host = job.getExecutionParams().getHost();
