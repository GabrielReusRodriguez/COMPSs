...
public class GATJob extends integratedtoolkit.types.Job implements MetricListener {
    ...
    public void submit() throws Exception {
        ...
        try {
            ...
            Tracer.freeSlot(getExecutionParams().getHost(), (Integer)jobDescr.getSoftwareDescription().getAttributes().get("slot"));
    ...
    public void processMetricEvent(MetricEvent value) {
        ...
        if (newJobState == JobState.STOPPED) {
        	if (tracing) {
            	...
            	Tracer.freeSlot(getExecutionParams().getHost(), slot);
            }
    ...
    private JobDescription prepareJob() throws Exception {
        ...
        if (tracing) {
        	...
        	lArgs.add(String.valueOf(method.getId() + 1)); // task id
